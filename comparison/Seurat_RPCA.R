...
parser$add_argument("--method", type = "character", default = "seurat_rpca")
...
rna_features <- SelectIntegrationFeatures(object.list = obj_rna.list, nfeatures = 4000)
obj_rna.list <- lapply(X = obj_rna.list, FUN = function(x) {
...
obj <- RunPCA(obj, reduction.name = "pca_rpca_rna", npcs = K)

# %% [markdown]
# ## Save embeddings

# %%
rpca <- Embeddings(object = obj, reduction = "pca_rpca_rna")
write.csv(rpca, pj(output_dir, "embeddings.csv"))

# %% [markdown]
# ## Visualization

# %%
obj <- RunUMAP(obj, reduction = 'pca_rpca_rna', dims = 1:K, reduction.name = "umap")
