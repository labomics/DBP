...
def sample_Gumbel_softmax(logits):
    return F.gumbel_softmax(logits, tau=1, hard=True, dim=1)
    ...
    for subset_id in data.keys():
        for varible in data[subset_id].keys():
            for m, v in data[subset_id][varible].items():
                ...
                if varible in ["z", "w", "x_trans", "x_impt"]:
                ...
                # elif varible in ["A"]:
                #     data[0][varible][m] = np.array(v, dtype=np.float32)
...
def get_pred_dirs(o, joint_latent, mod_latent, impute, batch_correct, translate, input):
    ...
    for subset_id in range(len(o.s_joint)):
        ...
        if joint_latent or mod_latent:
            ...
            dirs[subset_id]["EW"] = {}
            dirs[subset_id]["EA"] = {}
            # dirs[subset_id]["latent"] = {}
            dirs[subset_id]["A"] = {}
            if joint_latent:
                dirs[subset_id]["z"]["joint"] = pj(subset_dir, "z", "joint")
                dirs[subset_id]["w"]["joint"] = pj(subset_dir, "w", "joint")
                dirs[subset_id]["EW"]["joint"] = pj(subset_dir, "EW", "joint")
                dirs[subset_id]["EA"]["joint"] = pj(subset_dir, "EA", "joint")
                # dirs[subset_id]["latent"]["joint"] = pj(subset_dir, "latent", "joint")
                dirs[subset_id]["A"]["joint"] = pj(subset_dir, "A", "joint")
            if mod_latent:
                for m in o.combs[subset_id]:
                    dirs[subset_id]["z"][m] = pj(subset_dir, "z", m)
                    dirs[subset_id]["w"][m] = pj(subset_dir, "w", m)
                    dirs[subset_id]["A"][m] = pj(subset_dir, "A", m)
        if impute:
...
def save_rnk(fas_names, fas, rnk_dir):
    for i, fa_name in enumerate(fas_names):
        out_fa_dir = pj(rnk_dir, fa_name)
        mkdirs(out_fa_dir, remove_old=True)
        rnk_name_fmt = fa_name + ".rnk"
        rnk_name_fmt = pj(out_fa_dir, rnk_name_fmt)
        fa = fas[fa_name].sort_values(ascending=False)
        fa.to_csv(rnk_name_fmt, sep='\t', header=None)  
        
def save_topgenes(fas_names, fas, tg_dir):
    for i, fa_name in enumerate(fas_names):
        out_fa_dir = pj(tg_dir, fa_name)
        mkdirs(out_fa_dir, remove_old=True)
        tg_name_fmt = fa_name + ".csv"
        tg_name_fmt = pj(out_fa_dir, tg_name_fmt)
        fa = abs(fas[fa_name]).sort_values(ascending=False)
        fa.to_csv(tg_name_fmt, sep=',', header=None) 