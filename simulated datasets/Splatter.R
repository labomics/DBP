...
sim1_params <- setParams(
    ...
    batchCells     = c(1000, 1000, 1000, 1000, 1000) * 10,
    batch.facLoc   = c(0.10, 0.15, 0.12, 0.18, 0.15),
    batch.facScale = c(0.10, 0.08, 0.12, 0.10, 0.12),
...
batch_ncells <- c(300, 250, 220, 200, 180) * 10
# Proportions in each group in each batch that we want. Should sum to 1.
batch_props <- list(
    c(0.15, 0.22, 0.15, 0.13, 0.10, 0.05, 0.20),
    c(0.20, 0.33, 0.20, 0.10, 0.00, 0.08, 0.09),
    c(0.20, 0.30, 0.00, 0.20, 0.10, 0.10, 0.10),
    c(0.15, 0.20, 0.35, 0.15, 0.15, 0.00, 0.00),
    c(0.40, 0.05, 0.10, 0.10, 0.17, 0.12, 0.06)
    # c(0.25, 0.35, 0.15, 0.05, 0.00, 0.00, 0.20)
)
...
write.csv(as.matrix(counts(sim1_qc)), "simulations/counts.csv")
write.csv(rowData(sim1_qc), "simulations/genes.csv")
write.csv(colData(sim1_qc), "simulations/cells.csv")
